...
from general_tools import progress_bar,pload,pdump,reduce_range,find_nearest,extrapolate
...
def get_min_polyfit_quadratic(wlength,flux):
    ...
    c,b,a=np.polyfit(wlength-offset,flux,2)
    if c>0:
        x0=-b/(2*c)+offset
...
def get_min(wlength,mean,cov,nmc=False,fitting='cubic_poly',
    ...
    var1d=(len(cov.shape)==1)#true if 1D var supplied, false if 2D covarience
    ...
    failures=0
    MC_mins=[]
    last_min=wlength[lo:hi][np.argmin(mean[lo:hi])]
    l=lo
    r=hi
    for i in range(nmc):
        progress_bar(i,nmc)
        if not full_range:
            l,r=reduce_range(wlength[lo:hi],last_min-width,last_min+width,indices=True)
            l+=lo
            r+=lo
        if var1d:
            tflux=np.random.normal(mean,var)
        else:
            tflux=np.random.normal(np.zeros_like(mean),np.ones_like(mean))
            tflux=np.dot(cov,tflux)+mean
        if smoothing:
            if smoothing_matrix=='none':
                tflux=smooth_spectrum(wlength[l:r],tflux[l:r],var,uniform=uniform)#the l:r prevents measurement outside the desired range affecting the smoothing
                tflux=np.concatenate(([0]*l,tflux,[0]*(len(wlength)-r)))#pad with zeros so it is the correct length
            else:
                tflux=smoothing_matrix*np.matrix(tflux).T
                tflux=np.array(tflux.T)[0]  
        ...
        last_min=copy(x0)
        if VERBOSE:
            p.plot(wlength[l:r],tflux[l:r],color= colourdict['hex'][i],marker='x',ls='None')
