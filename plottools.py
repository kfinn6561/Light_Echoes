...
def wireframe(x,y,z,log=False,method='linear'): #creates a wireframe plot from a set of points using makegrid
...
def fwireframe(f,x_range,y_range,return_data=False):
    ...
    Y=[]
    Z=[]
    for x in np.linspace(x_range[0],x_range[1],100): #resolution is 100 by 100 but this could be changed by editing this line and the next for loop
        tempx=[]
        tempy=[]
        tempz=[]
    ...
    ax.plot_wireframe(X,Y,Z)
    if return_data:
        return (ax,X,Y,Z) #returns ax so that it is easy to update the plot
